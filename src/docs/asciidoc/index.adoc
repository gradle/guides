...
Creating multidex output for your project typically adds a significant amount of time to your build due to the merging process, so you should have a development flavor that targets a minimum SDK version of 21. This results in a faster build when working on the project code. Here's an example partial configuration:
...
You can find out more about this feature in the https://developer.android.com/studio/build/multidex.html#dev-build[Android Studio user guide].
