...
If you have tried all of the above except for the new software model and your configuration time is still too long, you may want to invest time in traditional profiling techniques to identify hotspots in your configuration code. Or consider investing that time in learning and using the new software model.
...
Beyond those settings, remember that the two big contributors to build times are configuration and task execution. You should now have a good idea of the pitfalls you need to avoid with regard to the configuration phase. Going forward, the new software model will remove the big structural costs of configuration and give you much more scope to tweak the performance of this aspect of your builds.

You currently have more control over task execution, since you can avoid running tasks or running them too often, and you can also code your own tasks to be as performant as possible. In the future, Gradle will offer more features to help with execution performance, such as parallel task execution. So you have plenty to look forward to.
