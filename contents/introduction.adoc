...
If the pulled changes are sufficiently independent, the developer can safely re-use outputs already generated on CI.
...
Using the local cache is a sort of one-to-one thing, while the HTTP cache typically has a one-to-many topology.
