...
Note that when using Gradle Enterprise you can easily find out which build produced an artifact coming from the build cache by clicking on the `FROM-CACHE` status.
