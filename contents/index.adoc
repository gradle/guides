...
There are technically two ways run the app. The first is to use Gradle's `run`
task which is added by
...
The tar and zip flies created in the `fairy/build/distributions` directory
...
continue ignoring that directory in it's default `copySpec`.
...
features of the Java 9 module system. The only thing left to do is bundle
the application with it's own isolated and optimized runtime environment.
