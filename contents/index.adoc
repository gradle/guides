...
Please be aware of a limitations when running TestKit-based functional tests from an IDE. TestKit determines the plugin classpath by running a specific Gradle task. To reflect changes to the classpath you will need to execute the `assemble` task. Some IDEs provide a convenience option to delegate "test classpath generation and execution" to the build. In IntelliJ you can find this option under _Preferences... > Build, Execution, Deployment > Build Tools > Gradle > Runner > Delegate IDE build/run actions to gradle_.
