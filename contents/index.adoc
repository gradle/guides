...
it will use a single shared classloader for all units of work.  This means that each unit of work must can potentially
affect each other through shared, static class state.  It also means that every unit of work must use the same version
of libraries that are on the buildscript classpath.  But what if you wanted the user to be able to configure the task to
run with a different (but compatible) version of the https://commons.apache.org/proper/commons-codec/[Apache Commons Codec]
library?  The Worker API allows you to do that, too.
