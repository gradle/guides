...
Last but not least we'll learn how the two DSLs inter-operate, and, go over a series of tips and troubleshooting techniques.
...
This is the best place where to find how to do this and what with each DSL  ; and it covers all Gradle features from link:#TODO[using plugins] to link:#TODO[customizing the dependency resolution behavior].


== Groovy meet Kotlin - WIP

// TODO redact

Kotlin and Groovy languages are very different beasts, each with its own strengths.

Kotlin the language is statically typed, has null safety built-int and comes with top-notch IDE tooling from auto-completion to refactoring and everything in-between..
...
// TODO more on other naming conventions for better IDE support
...
But, the declarative nature of the `plugins {}` block enables the Kotlin DSL to provide type safe discoverable accessors to what the applied plugins contribute (extensions, configurations etc...).
...
You can also use the imperative `apply` syntax:
...
Plugins fetched from another source than the link:https://plugins.gradle.org[Gradle Plugin Portal] may or may not be usable with the `plugins {}` block depending on how they have been published.
...
Composite builds https://github.com/gradle/gradle/issues/2528[do not expose plugin markers] yet.
...
== Multi project builds - WIP

// TODO redact

* project hierarchy == build scripts classpath
* impact of configure-on-demand, evaluationDependsOn() and evaluationDependsOnChildren()
...
Refactor towards the use of the `plugins {}` block. Use `pluginManagement {}` in `settings.gradle.kts` to control plugin resolution if needed.
...
Both link:https://kotlinlang.org/docs/reference/java-interop.html[calling Java from Kotlin] and link:https://kotlinlang.org/docs/reference/java-to-kotlin-interop.html[calling Kotlin from Java] are very well covered in the Kotlin reference documentation.
...
=== Static extensions - WIP

Both Groovy and Kotlin languages support extending existing classes, respectively with link:http://groovy-lang.org/metaprogramming.html#_extension_modules[Groovy Extension modules] and link:https://kotlinlang.org/docs/reference/extensions.html[Kotlin extensions].
...
=== Named parameters and default arguments - WIP

Both Groovy and Kotlin languages support named function parameters and default arguments.
They are link:https://kotlinlang.org/docs/reference/functions.html#named-arguments[natively] link:https://kotlinlang.org/docs/reference/functions.html#named-arguments[supported] by Kotlin.
...
To call a Kotlin function with named arguments from Groovy, you can't name arguments.
...
Some plugins expect simple closures, here with the link:https://plugins.gradle.org/plugin/com.jfrog.bintray[Bintray] plugin:
...
In other cases, like with the link:https://plugins.gradle.org/plugin/org.gretty[Gretty Plugin] when configuring farms, the plugin expects a delegate closure:
...
Also see the link:https://github.com/gradle/kotlin-dsl/tree/master/samples/groovy-interop[groovy-interop] sample.
...
[source, kotlin]
----
apply(from = "groovy-configuration.gradle")
----

.groovy-configuration.gradle
[source, groovy]
----
native {
    dynamic {
...
== Tips and troubleshooting - WIP

// TODO redact

* the `kotlinDslAccessorsReport` task prints the Kotlin code for accessing the currently available project extensions and conventions
...
* samples in user manual and guides
* Kotlin DSL samples
