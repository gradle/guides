...
== What you’ll need
 ...
 - A http://www.oracle.com/technetwork/java/javase/downloads/index.html[Java Development Kit] (JDK) version 8 or higher
...
----
$ gradle init --type java-library --package guidesamples
...
----
$ gradle init --type java-library --package guidesamples --dsl kotlin
...
The build script adds the {user-manual}java_library_plugin.html[java-library] plugin. This is an extension of the `java-base` plugin and adds additional tasks for compiling Java source code.
...
[source,java]
----
include::{gradle-outdir}/groovy-dsl-files/src/main/java/guidesamples/Library.java[tags=source-code]
----

The generated JUnit specification, `src/test/java/guidesamples/LibraryTest.java` is shown next:

.Generated  src/test/java/LibraryTest.java
[source,java]
----
include::{gradle-outdir}/groovy-dsl-files/src/test/java/guidesamples/LibraryTest.java[]
----
...
[NOTE]
====
All of this happens without any additional configuration in the build script because Gradle's `java-library` plugin assumes your project sources are arranged in a {user-manual}java_plugin.html#sec:java_project_layout[conventional project layout]. You can customize the project layout if you wish {user-manual}java_plugin.html#sec:changing_java_project_layout[as described in the user manual].
====
...
[source,java]
----
include::{gradle-outdir}/groovy-dsl-files/src/main/java/guidesamples/Library.java[tags=java-doc]
----
