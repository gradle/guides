...
== Building a typical Java project

As example, this post is going to discuss a Java-based project.
More specifically, the project is a Gradle plugin written in Java and tested with the http://spockframework.org/[Spock framework].
First, we'll get the project set up on your local machine before covering the same steps on CI.

Just follow these steps:

=== Clone https://github.com/gradle-guides/gradle-site-plugin[Gradle Site Plugin]
...
Navigate to the URL http://localhost:8080/[http://localhost:8080/] in the browser to render the Jenkins dashboard.
...
Enter the URL of the GitHub repository: `https://github.com/gradle-guides/gradle-site-plugin.git`.

image::jenkins-scm.png[]

Futhermore, create a "Build step" in the section "Build" by selecting "Invoke Gradle build script".
...
The build should finish successfully and render a "Gradle Build Scan" icon that brings you directly to the build scan.

image::jenkins-build-scan.png[align=center]
...
Please refer to the documentation on "build triggers" for more information.
...
* http://bmuschko.com/blog/jenkins-build-pipeline/[Modelling a Continuous Deployment pipeline for a Spring Boot application]
...
You are not using Jenkins?
No problem, many CI products tightly integrate with Gradle as a first-class citizen.
