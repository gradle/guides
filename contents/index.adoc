...
There are situations that require you to interact with a Gradle core plugin that uses `Convention`s. You can access the registered convention objects by calling the method `Project.getConvention()`. The specific convention implementation for a registered plugin can be retrieved by providing the plugin class via {api-reference}org/gradle/api/plugins/Convention.html#getPlugin(java.lang.Class)[`Convention.getPlugin(Class)`]. The <<convention-api-usage-example,sample code below>> references the `JavaPluginConvention` inspect configuration values exposed by the Java plugin.
