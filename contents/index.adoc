...
----
plugins {
include::{exampledir}/build.gradle[tags=declare-plugin]
}
----

NOTE: The latest version can be found on the https://plugins.gradle.org/plugin/com.gradle.plugin-publish[Plugin Portal].
...
----
<1> All information needed to publish a single or multiple plugins is configured in the `pluginBundle` extension block.
...
<4> Set the tags to be used for all plugins unless overridden in the `plugins` block.
...
----
pluginBundle {
include::{exampledir}/build.gradle[tags=plugin_definitions,indent=4]
}
----
...
If the https://docs.gradle.org/current/userguide/java_gradle_plugin.html[Java Gradle Plugin Development Plugin] is applied, it will automatically add a plugin configuration to the `pluginBundle` extension for each configured plugin and allow you to omit the `id`.
...
----
<1> Apply the Java Gradle Plugin Development Plugin.
<2> The names of the plugin in the `gradlePlugin` and `pluginBundle` extension blocks must match.
<3> The `id` specified in the `gradlePlugin` extension block is used by default for the plugin with the same name in the `pluginBundle` extension block, unless overridden there.

NOTE: Future versions of Gradle will support additional attributes, e.g. `displayName` and `description`, and allow to further reduce the boilerplate.
