...
=== Change 3 - Modify the `compileTestJava` task to locally alter the module
...
=== Change 4 - Modify the `test` task to consume the locally altered module
...
=== Change 5 (Optional) - Add `Automatic-Module-Name` manifest entries for all other projects
...
=== Change 3 - Update the `ServiceLoader` mechanism to the Java 9 syntax
...
feature of the java plugins, we are providing a plugin you can use to experiment
with Java 9 modules on your projects while we're still building the first-class
implementation.
...
in one shot. It *may* work for your project, but, you should definitely
