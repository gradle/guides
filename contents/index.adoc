...
In the above xref:build-scan-performance[] we can see that Configuration is taking over 14 seconds. Clicking on the _"Configuration"_&nbsp; tab at the top of the page will break this down for us.

image::build-scan-configuration-breakdown.png[title="Build scan configuration breakdown"]

Here we can see the scripts and plugins that were applied to our project in descending order of how long they took to apply. If we take the top offender, `script-b.gradle`, we can expand it up to see where this script is applied to our build
...
Here, it's easy to identify this particular issue; we should have a "Project configuration" time of zero, and this example shows we seem to be resolving dependencies too early in the lifecycle.
...
Again, build scans can help you investigate how well your tasks are caching. In the performance screen, we have a tab titled _"Build cache"_:

image::cache-performance.png[title="Inspecting the performance of the build cache for a build."]

This shows you statistics about how many tasks were fetched from a cache, which cache was used, along with download and unpack rates for these cache entries.
...
### Using build scans to trace un-cached tasks
