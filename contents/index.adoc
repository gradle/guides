...
We now apply Gradle’s `java` and `application` plugin and as well added the Gradle build scan plugin, with which we are able to create a build scan.
...
In the class above we create a new Spring Boot application and a `HelloGradleController` which returns `Hello Gradle!` when a `GET` request is processed on the root path of the application.
