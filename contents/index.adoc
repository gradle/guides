...
The `buildSrc` directory is one way to encapsulate build logic that you want to keep out of the project's build scripts. It's useful for custom task types as well as plugins. You can read about more about it in {user-manual}organizing_gradle_projects.html#sec:build_sources[the user manual].
