...
6. (Optional) Add `Automatic-Module-Name` manifest entries for all other
   projects.
...
produce a Java 9 module. The first five changes need to be made together, and
the sixth (optional) change can be made independently.
...
NOTE: This last option warrants a little more explanation. Why, didn't you just
set the `destinationDir` property of the `compileJava` task to the new value?
You are trying to arrange the compiled classes such that the `destinationDir`
can be used equally well as an element on a `--module-path` or a `--classpath`
when consuming the outputs of this task. Gradle automatically adds the
...
want to avoid causing the `compileClasspath` configuration from being resolved
...
to the module in the same package so that those test classes have access to
everything else which makes up the module under test.
...
`formula`, `fairy`, and `tale` subprojects in addition to the `bears`
subproject. You can see what the end results should look like in
`src/2-all-modules`.

That's it, now Gradle's `test` task will fail to compile unless you've
...
If you do comment out `canReachDefaultActor` and `@Ignore`
`canDynamicallyReachDefaultActor` the tests should pass, and you'll have
exactly the code in `src/2-all-modules`
...
By the end of Step 2, both of those mechanisms are relying on modules which
appear on the classpath which skips the modularity features of the Java 9
...
3. Remove the duplication of classes during Java compilation.
4. Update the `ServiceLoader` mechanism to the Java 9 syntax.
...
=== Change 1 - Modify the `run` task to use modules

To get the `run` task working with your new Java 9 modules, add the following
to the `build.gadle` file in the `fairy` project.
...
=== Change 2 - Modify the `startScript` task for both *nix and Windows to use modules
...
=== Change 3 - Remove the duplication of classes during Java compilation

Since the whole application is now using Java 9 modules both at compile and
runtime, we no longer need to copy the class files generated during the
`compileJava` task into their classpath-friendly location. Nor, should we
consume them from that location when building the jar file.

You can remove the `doLast` block from the `compileJava` task so that it looks
like this:

.build.gradle
[source,groovy]
----
include::{appdir}/build.gradle[tags=compileJava]
----

You must also tell the `jar` task to consume the classes from the directory
named the same as the module in the `main.java.outputDir` and tell it to
continue ignoring that directory in its default `copySpec`.

.build.gradle
[source,groovy]
----
include::{appdir}/build.gradle[tags=jar]
----

=== Change 4 - Update the `ServiceLoader` mechanism to the Java 9 syntax
...
`pigs/src/main` as the new mechanism doesn't require the `META-INF/services`
files.
...
which declare `provides org.gradle.fairy.tale.Tale`.

== Summary

At this point, your application is taking advantage of all most all the
