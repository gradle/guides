...
Build scans are a feature provided by Gradle that aggregate information across multiple build runs and make those diagnostics available to you online as a report. You can
...
1. Apply the _build-scan_ plugin https://scans.gradle.com/get-started[as described on Gradle.com]
...
A profile report will help you identify which projects take the most time to configure, but that’s all. The advantage of using a build scan is that it lets you drill down into this configuration time, and see what is causing any slowness.

In the above xref:build-scan-performance[], we can see that Configuration is taking over 14 seconds. Clicking on the _"Configuration"_&nbsp; tab at the top of the page will break this stage down into its component parts.

image::build-scan-configuration-breakdown.png[title="Build scan configuration breakdown"]

Here, we can see the scripts and plugins that were applied to the project in descending order of how long they took to apply. Taking the top offender, `script-b.gradle`, it can be expanded to see where this script is applied to the build.

image::script-b-application.png[title="Showing the application of script-b to our build"]

So we can see that this script is applied once, by the project `:app1` from inside of that project's `build.gradle` file, and would be a good place to start trying to optimize our build.

Hopefully you can see how the ability to drill into the data provided by build scans can really help in optimizing a build.
...
In our above build scan example, we can see that `script-a.gradle` is applied to 3 projects inside the build:

image::script-a-application.png[title="Showing the application of script-a to our build"]

This script takes a second to run, and we can see it is applied to 3 projects from the root `build.gradle` file, and so is introducing a 3 second delay to our configuration.
...
The performance page of build scans explicitly shows how dependency resolution time is split across project configuration and task execution:

image::bad-dependency-resolution.png[title="Bad dependency resolution at configuration time."]

Here, it's easy to identify this particular issue; we should have a _"resolved dependencies during project configuration"_&nbsp; time of zero, and this example shows we seem to be resolving dependencies too early in the lifecycle.  Also on the _"Performance"_&nbsp; screen is a _"Settings and suggestions"_&nbsp; tab which will show you which dependencies were being resolved during project configuration.

If you are using the profiler, a similar (but much less detailed) result can be seen by running;
...
As can be seen above, a task which we expected to be incremental (and up-to-date) wasn't, and the build scan shows us why -- one of its inputs _"timestamp"_&nbsp; has changed, and is forcing the task to be re-run.
...
We have a complete guide about https://guides.gradle.org/using-build-cache/[using the build cache]. It covers the different scenarios caching can improve, and detailed discussions of the different caveats you need to be aware of when enabling caching for a build.
...
This shows you statistics about how many tasks were fetched from a cache, which cache was used, along with transfer and pack/unpack rates for these cache entries.

There is also a _"Task execution"_&nbsp; tab which shows details including cacheablity on the tasks that were executed. Clicking on any of the categories wil take you to the Timeline screen with just tasks of that category highlighted.
