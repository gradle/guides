...
Every Gradle plugin project should apply the {user-manual}java_plugin.html[plugin development plugin] to reduce boilerplate code. By applying the plugin development plugin, the project is preconfigured for the use of TestKit. Let's point the plugin to the custom source set containing functional tests.
