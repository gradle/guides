...
You can see that this plugin is applied once, by the project `:app1`, from inside of that project's `build.gradle` file.
...
Here, it's easy to identify this particular issue; you should have a _"resolved dependencies during project configuration"_&nbsp; time of 0&nbsp;seconds, and this example shows the build seems to be resolving dependencies too early in the lifecycle.  Also on the _"Performance"_&nbsp; screen is a _"Settings and suggestions"_&nbsp; tab which will show you which dependencies were being resolved during project configuration.
...
As can be seen above, build scans show why a task was not up-to-date -- one of its inputs _"timestamp"_&nbsp; has changed, and is forcing the task to be re-run.
