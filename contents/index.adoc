...
In this guide, we'll discuss how to configure link:https://www.jetbrains.com/teamcity/?fromMenu[TeamCity] for a typical Gradle project.
...
* A TeamCity installation (setup steps explained in this post)
...
=== Clone the https://github.com/gradle-guides/gradle-site-plugin[Gradle Site Plugin] repository
...
The project provides the https://docs.gradle.org/current/userguide/gradle_wrapper.html[Gradle Wrapper] as part of the repository.
...
The following section will describe how to build the project with the help of TeamCity.
...
Unpack the `TeamCity<version number>.tar.gz` archive, for example, using the WinZip, WinRar or a similar utility under Windows, or the following command under Linux:
[listing]
----
...
TeamCity comes bundled with Gradle plugin, so you do not need to install plugins additionally.

On the *Administration | Projects* page click _Create project_,
use the option _From the repository URL_ and enter the URL of the GitHub repository: `https://github.com/gradle-guides/gradle-site-plugin.git`.

image::teamcity-create-project.png[]
...
image::teamcity-build-step.png[]

The build step is added to the build configuration:

image::teamcity-step-added.png[]
...
image::teamcity-results.png[]

You can view the tests right here in TeamCity:

image::teamcity-test.png[]

The information on parameters and environment of the build is available
on the _Parameters_ tab of the build results.

The link to the link:https://scans.gradle.com[build scan] for the given build is available
in the build log:

image::teamcity-log-link.png[]
