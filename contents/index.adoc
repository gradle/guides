...
NOTE: Gradle classloader injects setter methods side-by-side with the all `Property` type getter methods. It allows developers to simplify code like `obj.prop.set 'foo'` to `obj.prop = 'foo'` in the Groovy DSL.
