...
Please be aware of a limitation when running TestKit-based functional tests from an IDE. TestKit determines the plugin classpath by running a specific Gradle task. You will need to execute the `assemble` task to initial generate the plugin classpath or to reflect changes it. Some IDEs provide a convenience option to delegate the "test classpath generation and execution" to the build. In IntelliJ you can find this option under _Preferences... > Build, Execution, Deployment > Build Tools > Gradle > Runner > Delegate IDE build/run actions to gradle_.
