...
Here, it is possible to see the scripts and plugins that were applied to the project in descending order of how long they took to apply. Taking, as an example, the plugin `com.example.bar` which was applied once but took seven seconds, the row can be expanded to see how and where this plugin was applied to the build.

image::bar-plugin-application.png[title="Showing the application of com.example.bar to the build"]

So it can be seen that this plugin is applied once, by the project `:app1` from inside of that project's `build.gradle` file.
...
In our above build scan example, we can see that the plugin `com.example.foo` is applied to 3 projects inside the build:

image::foo-plugin-application.png[title="Showing the application of com.example.foo to the build"]

This plugin takes a three seconds to run, and as it is applied to 3 projects from the root `build.gradle` file, it is introducing a 3 second delay to the configuration phase.
