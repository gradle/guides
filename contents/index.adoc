...
Some of the Gradle core plugins expose configurability with the help of a so-called {api-reference}org/gradle/api/plugins/Convention.html[Convention]. `Convention` is the preceding concept of an extension and serves a similar purpose as an extension. Please avoid using the `Convention` concept when writing new plugins. The long term plan is to migrate all Gradle core plugins to use extensions and remove the `Convention` concept altogether.

There are situation that require you to interact with a Gradle core plugin that uses then `Convention` concept. You can access the registered convention objects by calling the method `Project.getConvention()`. The specific convention implementation for a registered plugin can be retrieved by providing the plugin class via {api-reference}org/gradle/api/plugins/Convention.html#getPlugin(java.lang.Class)[`Convention.getPlugin(Class)`]. The <<convention-api-usage-example,sample code below>> references the `JavaPluginConvention` inspect configuration values exposed by the Java plugin.
...
Instead of using a plain `String` type, the extension defines the field `serverUrl` with type `Property<String>`. The field is initialized in the constructor of the class. It's state can be set via the exposed setter methods.
