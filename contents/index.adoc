...
DSLs exposed by a plugin come in different shapes and sizes. Sometimes you might want to expose a way for users to define multiple, named data objects of the same type. Let's consider the following build script for illustration purposes.
...
Gradle exposes the convenience method {api-reference}org/gradle/api/Project.html#container(java.lang.Class)[Project.html#container(java.lang.Class)] to create a container of data objects. The only parameter it takes is the class representing the data. Creating an instance of type {api-reference}org/gradle/api/NamedDomainObjectContainer.html[NamedDomainObjectContainer] is not enough though. You will also have to expose it to the end user. You can do so by adding it to the extension container with a specific name.
...
public class ServerEnvironmentPlugin implements Plugin<Project> {
    public void apply(Project project) {
        NamedDomainObjectContainer<ServerEnvironmentContainer> serverEnvironmentContainer = project.container(ServerEnvironmentContainer.class);
        project.getExtensions().add("environments", serverEnvironmentContainer);
    }
}
----

It's fairly easy to navigate the data that has been captured by the end user. Let's say you'd want to iterate over the values in the build script then you could do so by getting a handle to the named extension `environments`. The following task demonstrates the use case.

.build.gradle
[source,groovy]
----
task printEnvironmentUrls {
    doLast {
        environments.each {
...
It's very common for a plugin to post-process the captured values within the plugin implementation e.g. to configure tasks. Please consider the following section for more information.
