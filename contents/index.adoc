...
This guide demonstrates how to create a https://kotlinlang.org[Kotlin] library targeted for the JVM. We will use https://github.com/gradle/kotlin-dsl[Gradle's Kotlin DSL] for build scripts.
...
You'll notice that we're using the new `implementation` dependency configuration. We're declaring that we do not expose any of the Kotlin standard library through our API. This has https://blog.gradle.org/incremental-compiler-avoidance[architectural and performance benefits].

We're nearly ready to start testing and packaging our software.

=== Step 1.2: Digging deeper with a build scan
...
http://junit.org/junit4/[JUnit 4] is the simplest of testing libraries to add to our project. If you happen to prefer the http://spekframework.org/[Spek framework] for Kotlin-based specifications, check out the http://spekframework.org/docs/latest/#_gradle_kotlin_script[Spek docs].
...
.src/test/kotlin/org/example/MyLbraryTest.kt
[source,kotlin]
----
...
=== Step 3.1: Apply the Dokka plugin
...
=== Step 3.2: Configure Dokka plugin
...
We could optionally also generate a `"sources"` JAR using the same pattern from `java.sourceSets.getByName("main").allSource`.
...
=== Step 4.1: Apply the maven-publish plugin
...
=== Step 4.2: Set project coordinates

We need to set coordinates in our build script for publishing.
...
----
<1> Configure project coordinates to `org.example:my-kotlin-library:0.0.1`
...
[source,groovy]
----
include::{samplescodedir}/step-4/settings.gradle[]
----

=== Step 4.3: Configure Maven publication
