...
* IntelliJ IDEA or Android Studio might spawn up to 3 Gradle daemons when editing scripts. One per each type of scripts: project scripts, settings scripts and initialization scripts. Also note that there are reports that on some builds with slow configuration time the IDE may feel unresponsive when editing scripts.
...
* The performance characteristics of the Groovy and Kotlin DSLs are similar in "everything is up to date" scenarios. Statically compiled build logic is also faster at configuration as well as execution time. The performance difference can range from neglectable to substantial.
* However, first use, on clean checkouts or ephemeral CI agents for example, link:{kotlin-dsl-issues}902[is slower]. The same applies to the scenario in which something in the _buildSrc_ directory changes, which invalidates build-script caching.
