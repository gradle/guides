...
The plugin provides a task, called `init`, that generates the project. The plugin also uses the (also built-in) link:{language-reference}org.gradle.api.tasks.wrapper.Wrapper.html[`wrapper` task] to create a link:{user-manual}gradle_wrapper.html[Gradle wrapper script], `gradlew`.
...
From inside the new project directory, run the `init` task and select the `swift-application` project type followed by Swift language when prompted. For the other questions, press enter to use the default values.
...
See the link:{user-manual}building_swift_projects.html#sec:custom_swift_source_set_paths[customizing Swift source] section of the Swift chapter in {user-manual-name} as well as the https://github.com/gradle/native-samples/tree/master/swift/swift-package-manager[custom source layout sample] for a demonstration.
...
* The `swift-application` plugin automatically generates an application component configurable via the link:{language-reference}org.gradle.language.swift.SwiftApplication.html[application] extension DSL.
* The `xctest` plugin automatically generates a bundle (on macOS) or an executable (on Linux) component that depends on the application.
