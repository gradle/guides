...
Tradeoffs are inherent in any engineering decision, but using disjointed tools can slow down very capable development teams. Gradle provides scaffolding to integrate with everything, even Webpack and other JavaScript development tools. More information about managing Webpack with Gradle (including hot reloading) is covered in an in-depth topical guide (coming soon).
...
Now executing `webpack` with no changes to web assets will avoid unnecessarily invoking Webpack:
...
== Next Steps
Congratulations! You now have a Gradle task that executes Webpack, but only when web assets change.

For medium and large-sized projects, it is more important to take advantage of computation and information computers have created, in order to save time — this is one of the fundamental goals of https://gradle.com/enterprise[Gradle Enterprise].

Chances are, your needs are more complex. If so, the topical guide Managing JavaScript with Gradle (coming soon) is for you. Happy https://webpack.js.org/[Webpack] bundling!
