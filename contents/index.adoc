...
<8> Native libraries are specified as a component of type {language-reference}org.gradle.nativeplatform.NativeLibrarySpec.html[NativeLibrarySpec] and defined by a name - `greeter` in this case.
<9> Native executables are specified as a component of type {language-reference}org.gradle.nativeplatform.NativeExecutableSpec.html[NativeExecutableSpec] and defined by a name - `main` in this case.
...
It then runs the testsuite binary as well. The test binary in this case will be named `greeterTest` and the task will be named `runGreeterTestGoogleTestExe`.
