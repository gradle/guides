...
This guide walks you through the process of using Gradle's Buidl Init plugin to produce a JVM library which is suitable for consumption by other JVM libraries and applications.
...
The build file adds the {user-manual}java_library_plugin.html[java-library] plugin. This is an extension of the `java-base` plugin and adds additional tasks for compiling Java source code.
...
[source,java]
----
include::{initexampledir}/src/main/java/Library.java[tags=source-code]
----

The generated Spock specification, `src/test/java/LibraryTest.java` is shown next:

.Generated  src/test/java/LibraryTest.java
[source,java]
----
include::{initexampledir}/src/test/java/LibraryTest.java[]
----
...
Congratulations, you have just completed the step from creating a Java library! You can can now customise this to your own project needs.
...
The code generated by the Build Init plugin already placed a comment on the `Libraryjava` file. Modify the comment to become Javadoc markup.

.src/main/scala/Library.java
[source,java]
----
include::{initexampledir}/src/main/java/Library.java[tags=java-doc]
----
...
You can view the generated Javadoc by opening the HTML file, located at `build/docs/javadoc/index.html`.
