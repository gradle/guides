...
<3> Native libraries are specified as a component of type {language-reference}org.gradle.nativeplatform.NativeLibrarySpec.html[NativeLibrarySpec] and defined by a name - `greeter` in this case.
<4> Native executables are specified as a component of type {language-reference}org.gradle.nativeplatform.NativeExecutableSpec.html[NativeExecutableSpec] and defined by a name - `main` in this case.
<5> The `main` binaries use the `greeter` {c++} library and is specified using `cpp.lib`
...
----
<1> Gradle does not show the compiler commands or only show an excert of the compiler output if the compilation failed in order to keep the output from Gradle itself clean. But the compiler output can be found in `build/tmp/compileGreeterStaticLibraryGreeterCpp/output.txt` and the compiler flags can be found in `build/tmp/compileGreeterStaticLibraryGreeterCpp/options.txt`
