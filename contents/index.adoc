...
Starting with Gradle 4.3 you can generate a build scan without any additional configuration in your build script. By using the command line option `--scan`, the latest build scan plugin is applied automatically. The build will ask you to accept the license agreement on the command line. The following console output demonstrates the behavior.
...
This mechanism makes it very easy to generate ad-hoc, one-off build scans without having to configure the plugin in your build. If you need more fine-grained configuration, then you can configure the build-scan plugin in a build or init script as described in the following sections.
