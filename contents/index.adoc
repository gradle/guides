...
We'll then go a bit deeper to understand how the Groovy DSL is dynamic, how the Kotlin DSL is static, how they inter-operate, and, go over a series of tips and troubleshooting techniques.
Last but not least, we'll discuss possible migration strategies in light of the structure of builds.
...
// TODO about the importance of script classpath & the plugins {} block
// TODO project hierarchy == classpath hierarchy
// TODO buildSrc -> settings -> projects tree
...
[TIP]
You can switch from Groovy script excerpts to Kotlin ones to see the differences between them by clicking the tabs on top of them.
...
// TODO on using pluginManagement {} to use the plugins block
...
In both scripts above, a distribution named `"samples"` is created and configured on the `distributions` extension which is a link:{user-manual}custom_plugins.html#sec:maintaining_multiple_domain_objects[collection of objects] for which both the Groovy DSL and the Kotlin DSL provide syntactic helpers.
