...
You now have a project that can copy sources and create a Zip archive.

== Explore and debug your build

Let's see what else we can do with Gradle in our new project. A full {user-manual}command-line-interface.html[reference to the command-line interface] is available as well.
...
By using the `--scan` option or applying the build scan plugin to your project, you can create a build scan at scans.gradle.com free of charge. Publishing build scans to scans.gradle.com transmits link:https://docs.gradle.com/scans/#captured_information[this] data to Gradle's servers. If you want to keep your data on your servers, check out link:https://gradle.com/enterprise[Gradle Enterprise].
