...
$ ./gradlew assemble --build-cache <2>

BUILD SUCCESSFUL in 9s
15 actionable tasks: 13 executed, 2 up-to-date
$ ./gradlew clean assemble --build-cache <3>

BUILD SUCCESSFUL in 1s
15 actionable tasks: 3 executed, 10 from cache, 2 up-to-date
----
<1> We want to start with an empty local cache.
<2> Build the project once to let it populate the cache.
<3> Build it again. This time everything cacheable should load from the just populated cache.
...
A task is considered _relocatable_ if its output can be reused when it is executed in a different location.
Tasks that should be relocatable but aren't are usually caused by some of the task's inputs containing absolute paths.

To shake these problems out, check out the same commit of your project in two different directories on your machine.
...
15 actionable tasks: 13 executed, 2 up-to-date
$ cd ~/checkout-2 <4>
$ ./gradlew clean assemble --build-cache <5>
...
<3> Run a clean build to populate the cache.
<4> Go to the other checkout directory.
<5> Run a clean build again.
...
This means that Gradle Enterprise has the necessary data to diagnose the cache miss when comparing two build scans:
