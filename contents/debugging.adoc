...
To quickly compare two builds, open the build scans of interest, select the _"adjacent scan"_ icon at the top of the page, and
click _"compare"_ next to the build you want to compare it to.

[.screenshot]
image::adjacent-build-comparison.png[]

Gradle Enterprise has the necessary data to diagnose the cache miss when comparing two build scans, and will show you the
properties that caused a different cache key to be generated.
