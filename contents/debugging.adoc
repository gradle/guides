...
The expected outcome is that that all actionable tasks that produce file outputs are up-to-date.
...
[WARNING]
====
When running this test make sure that you have no `remote` cache configured, and pushing to the `local` cache is enabled.
...
[WARNING]
====
Like with the previous test, you should have no `remote` cache configured, and pushing to the `local` cache should be enabled.
====
