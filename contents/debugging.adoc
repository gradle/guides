...
The goal is to achieve <<stable_task_inputs,stable task inputs>> for cacheable tasks.
...
This can be problematic, see the <<custom_actions,section>> on adding custom actions to your tasks.
====
...
As described in <<java_version_tracking,before>>, the Java version can also influence the output of the Java compiler.
...
In order to solve this problem see the <<volatile_outputs,section on non-repeatable task outputs>>.
