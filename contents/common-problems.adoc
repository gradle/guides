...
You should make sure to configure `autocrlf=false` across your build machines to ensure optimal build cache usage.
...
The story is different if an other task depends on the volatile outputs of a task.
...
The details what level of understanding Gradle uses for detecting changes to classpaths see the {user-manual}more_about_tasks.html#sec:task_input_using_classpath_annotations[userguide].
...
Note that it can be challenging to have tasks reproduce exactly the same byte per byte output for the same inputs.
...
Special care should also be taken for code generation tasks which may produce <<volatile_outputs>>.
