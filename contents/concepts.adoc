...
Timestamps and incremented sequences like the CI build number can sneak in as inputs when such information is added to the build's version number.
A version number like `3.2-__TIMESTAMP__` can appear in a `version.properties` resource file.
Anything that depends on that on that file will have a fresh set of inputs with every build.

Another example for unstable inputs is the commit ID from version control: maybe your version number is generated via `git describe`, maybe you include the commit hash directly in `version.properties` or a JAR manifest attribute.
...
Task properties that have no path sensitivity declared are considered non-relocatable
