...
As we said before, how much of your build gets loaded from cache depends on many factors. In this section we'll review some of the tools that are essential for well-cached builds. https://gradle.com/build-scans[Build scans] give valuable insights to your builds and we will use them throughout this guide.
...
Build scans show tasks where caching was disabled for overlapping outputs in the timeline:
