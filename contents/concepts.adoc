...
Artifacts in the build cache are uniquely identified by a _{user-manual}build_cache.html#sec:task_output_caching_details[build cache key]._
...
Moving the files around wouldn't have an effect on the result of the compilation, but we rely on the relative path to find other classes in the same package.
