...
TIP: The plugin is link:https://plugins.gradle.org/plugin/org.gradle.plugins.site[available on the Gradle plugin portal] for public consumption.

== Usage example

The plugin can be applied with by identifier `gradle.site`. Default values can be configured with the help of the provided extension `org.gradle.plugins.site.SitePluginExtension`. To generate the web page run the task named `generateSite` e.g. `gradle generateSite`.
...
site {
    ...
    vcsUrl = "https://github.com/my/repo.git"
}
```
