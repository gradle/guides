= Basic Build Scans image:https://travis-ci.org/{repo-path}.svg?branch=master["Build Status", link="https://travis-ci.org/{repo-path}"]
...
== Step 1: Select A Sample Project
...
If you already have a `plugins` block, add the build-scan plugin shown first. Adding it below any existing plugins will still work, but will miss useful information.

NOTE: When this guide was written, the latest version of the build scan plugin was 1.6. The latest version of the plugin can be found from the https://plugins.gradle.org/plugin/com.gradle.build-scan[Gradle Plugin Portal].

Using the `plugins` block in this way will not only download the plugin, it will apply it as well.

== Step 3: Add the License Agreement
...
== (Optional) Step 6: Enable Build Scans For All Builds
...
Additional information can be found in the Build Scans User Manual found at https://docs.gradle.com/scans/ .
