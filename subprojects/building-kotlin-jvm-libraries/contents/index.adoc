...
If you get stuck at any point, you can check out https://github.com/gradle/guides/tree/master/subprojects/building-kotlin-jvm-libraries/samples/code/my-kotlin-library[the completed example].
...
<2> Declare a link:{language-reference}org.gradle.api.artifacts.dsl.RepositoryHandler.html[dependency repository] on Bintray jcenter
...
Now our library is ready to share with the world. We're going to publish it locally with the link:{user-manual}publishing_maven.html[maven-publish plugin] just to keep things simple, but another popular choice is the https://plugins.gradle.org/plugin/com.jfrog.bintray[Gradle Bintray Plugin]. Their documentation has https://github.com/bintray/bintray-examples/tree/master/gradle-bintray-plugin-examples[several samples] you can use if you're interested.
...
Congratulations! You've successfully built a Kotlin library, tested it, documented it, and packaged it as a JAR with docs and sources. The https://github.com/gradle/guides/tree/master/subprojects/building-kotlin-jvm-libraries/samples/code/my-kotlin-library[complete sample project is on GitHub].
...
We hope you found this fun and enlightening. Let us know what you thought of this guide via https://twitter.com/gradle[@gradle on Twitter].
 ...
 - There are lots of https://github.com/gradle/kotlin-dsl/tree/master/samples[Kotlin DSL samples] available in the gradle/kotlin-dsl repository on GitHub, as well as additional link:{user-manual}kotlin_dsl.html[getting started docs]
