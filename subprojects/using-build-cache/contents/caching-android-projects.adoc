...
This defeats link:{user-manual}more_about_tasks.html#sec:up_to_date_checks[incremental build] (i.e., `UP-TO-DATE` checks), but this issue is mitigated via use of the local build cache.
...
This defeats link:{user-manual}more_about_tasks.html#sec:up_to_date_checks[incremental build], because each build will be unique.
...
There is a simple workaround for this, based on link:{user-manual}kotlin_dsl.html#using_a_groovy_script[this advice] from the Kotlin DSL primer.
