...
You will create a Gradle task that bundles web assets with https://webpack.js.org/[Webpack] in a way that leverages Gradle's link:{user-manual}more_about_tasks.html#sec:up_to_date_checks[up-to-date checks] and link:{user-manual}build_cache.html[build cache].
...
Command-line tools can be invoked through Gradle link:{language-reference}org.gradle.api.tasks.Exec.html[Exec] tasks.
...
You are now able to execute Webpack through Gradle. Let's move on to only running Webpack when you _need_ to, through Gradle's link:{user-manual}more_about_tasks.html#sec:up_to_date_checks[up-to-date checks] (AKA incremental build).
...
As of Gradle 4.0, Gradle can avoid work that has already been done on different VCS branches or by other machines via the link:{user-manual}build_cache.html[Build Cache].
...
It is strongly advised to use a link:{user-manual}custom_tasks.html[custom task class] when writing cacheable tasks. An example of one for Webpack is provided in the Managing JavaScript topical guide (coming soon). Furthermore, you may want to declare property names for better diagnostics.
...
include::contribute[repo-path="gradle-guides/running-webpack-with-gradle"]
