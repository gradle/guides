...
NOTE: The directories `src/integTest/groovy` and `src/functTest/groovy` are not based on an existing standard convention for Gradle projects. You are free to choose any project layout that works best for you.
...
Gradle does not dictate the use of a specific test framework. Popular choices include link:https://junit.org/junit4/[JUnit], link:https://testng.org/[TestNG] and link:https://spockframework.org/[Spock]. Once you choose an option, you have to add its dependency to the compile classpath for your tests. The following code snippet shows how to use Spock for implementing tests.
...
This section discusses representative implementation examples for unit, integration and functional tests. All test classes are based on the use of Spock though it should be relatively easy to adapt the code to a different test framework. You can find all classes in the https://github.com/gradle/guides/tree/master/subprojects/testing-gradle-plugins/samples/code/url-verifier-plugin[source code repository for this guide]. Please revisit the section <<testing-pyramid,the testing pyramid>> for a formal discussion of the definition of each test type.
...
include::contribute[repo-path="gradle-guides/testing-gradle-plugins"]
